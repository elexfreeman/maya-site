                ...
                <div v-for="item in items" :key="item.id" class="service-card">
                    <div class="service-image">
                        <div
                            ...
                            :style="{
                                backgroundImage: `url(${item.img || '/img/IMG_8886.jpg'})`,
                            }"
                        >
...
const placeholder: Service[] = [
    {
        ...
        description:
            "Цветы, травы и веточки в акварели. Лёгкие композиции для интерьера и подарков.",
        img: "/services/botanic.jpg",
        tags: "Оригиналы и принты, Натуральные оттенки, Индивидуальные сюжеты",
        tagsList: [
            "Оригиналы и принты",
            "Натуральные оттенки",
            "Индивидуальные сюжеты",
        ],
    },
    {
        id: 2,
        caption: "Пейзажи",
        description:
            "Спокойные акварельные виды: море, горы, закаты. Лёгкая цветовая гамма для домашнего уюта.",
    ...
    },
    {
        ...
        description:
            "Печать моих работ на плотной дизайнерской бумаге. Небольшие форматы — для приятных поводов.",
        img: "/img/IMG_8886.jpg",
    ...
    },
    {
        ...
        caption: "Пастель",
        description:
            "Тёплые и мягкие пастельные работы: портреты, натюрморты и уютные сюжеты для интерьера.",
        img: "/img/IMG_8886.jpg",
...
const items = computed<Service[]>(() => {
    const base = (
        data.value?.items?.length ? data.value.items : placeholder
    ) as Service[];
    const hasPastel = base.some(
        (i) => i.caption && i.caption.toLowerCase() === "пастель",
    );
